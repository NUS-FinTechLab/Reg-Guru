...
DESTINATION_DIR = os.path.join(
    CURRENT_DIR, "..", "..", "..", "..", "data_ingestion", "raw", "sg", "sso"
)
...
class SsoScraper(BaseScraper):
    ...
    def __init__(
        ...
        ds_description=DEFAULT_DS_DESC,
    ):
    ...
    def extract_meta_from_webpage(self, weblink):
        ...
        desktop_timeline = soup.find(
            "div", class_="desktop-timeline hidden-xs hidden-sm"
        )
        latest_valid_date = (
            desktop_timeline.find_all("div", class_="timestamp")[-1]
            .find("a", class_=None)
            .text.strip()
        )  # class_=None avoids the file-download link
        latest_published_date = desktop_timeline.find_all(
            "a", class_="timeline-popover"
        )[-1]["data-date"]

        return {
            "valid_date": latest_valid_date,
    ...
    def extract_documents_from_page(self, page_url):
        ...
        for row in tqdm(rows):
            try:
                ...
                # Extract PDF link
                pdf_href = data_cells[1].find("a", class_="non-ajax file-download")[
                    "href"
                ]
                if pdf_href:
                    ...
                    document = {
                        ...
                        "weblink": weblink,
                        "pdf_href": (
                            urljoin(self.base_url, pdf_href)
    ...
    def scrape(self):
        ...
        while current_url:
            try:
                ...
                )  # We'll modify extract_documents_from_page to store this
                if not html:
                    continue
                soup = BeautifulSoup(html, "html.parser")
    ...
    def log_into_database(self, documents):
        ...
        if not hist_df.empty:
            hist_df["valid_date"] = pd.to_datetime(
                hist_df["valid_date"], errors="raise"
            ).apply(lambda x: int(x.timestamp()) if pd.notnull(x) else None)
            docs = pd.DataFrame(documents).merge(
                hist_df, how="all", on="doc_id", suffixes=("", "_lastest")
            )

            # Documents need to be updated
            updated_docs = docs[
                docs["valid_date"] > docs["valid_date_latest"]
            ]  # Need a module to handle updated docs, likely mark prev doc as obsolete and insert new doc as a record
            ...
            for _, doc in updated_docs.iterrows():
                try:
                    ...
                    print(
                        f"Error marking document {doc['doc_id']} as superseded: {str(e)}"
                    )
            ...
            for _, doc in disappeared_docs.iterrows():
                try:
                    ...
                    print(
                        f"Error marking document {doc['doc_id']} as disappeared: {str(e)}"
                    )

            # Insert new or updated documents
            docs_to_insert = docs[
                docs["valid_date"].notna()
                & (docs["valid_date"] > docs["valid_date_latest"].fillna(0))
            ]
        else:
        ...
        if docs_to_insert.empty:
            ...
            self.log_id = self.db_client.execute(
                query, (self.ds_id, self.ds_description, 1)
            )[0][0]
            ...
            values = [
                ...
                for _, doc in docs_to_insert.iterrows()
            ]
            for value in values:
                try:
    ...
    def store_documents(self, log_id):
        ...
        if len(new_entries) > 0:
            ...
            for entry in new_entries:
                ...
                if bucket_name:
                    file_obj_key = f"{new_feed_folder}/{doc_id}.pdf"
                    try:
                        downloadPdftoS3(pdf_url, file_obj_key)
                    except Exception as e:
                        print(f"Error uploading PDF {pdf_url} to S3: {str(e)}")
                else:
                    local_dir = os.path.join(DESTINATION_DIR, str(log_id))
                    os.makedirs(local_dir, exist_ok=True)
                    local_path = os.path.join(local_dir, f"{doc_id}.pdf")
                    try:
                        downloadPdf(pdf_url, local_path)
                    except Exception as e:
            ...
            if bucket_name:
                ...
                print(
                    f"{len(new_entries)} documents saved locally under {DESTINATION_DIR}/{log_id}."
                )
        else:
